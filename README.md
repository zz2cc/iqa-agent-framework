...
- 后续相同参数的调用**秒级命中**，**零 API 消耗**、零费用——直接返回已保存的响应文本，不再发出网络请求
- **换言之，在当前这份代码和数据上，你只需要跑一次**。重跑、调试、换参数——已完成的步骤自动跳过，不会重复花钱

### 5.3 缓存的边界

缓存保存的是**某次具体的 API 响应**，不是"标准答案"。同一份缓存在本项目内部可以安全复用（相同的模型名、相同的 prompt、相同图片、T=0），但跨环境迁移时需要注意：

- API 服务端的模型权重可能被供应商静默更新，导致同一 prompt 的输出发生变化
- 不同 API Key 的账户配置可能影响响应
- 因此 `runs/cache/` 不入 git——其他研究者在自己的环境里应当通过重跑脚本生成自己的缓存
...
| 结果可复现 | 所有评分结果落盘为 scores.csv，经 GitHub 克隆验证全部 22 个指标精确匹配 |
